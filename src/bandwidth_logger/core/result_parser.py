...
# --------------------------------------------------------------------------
# speedtest-cli (open-source fallback)
# --------------------------------------------------------------------------


def parse_speedtest_cli_json(stdout: str, *, engine_version: str | None = None) -> EngineMeasurement:
    """Parse ``speedtest-cli --json``.

    This engine already reports bits per second, and supplies neither jitter
    nor packet loss -- those stay NULL rather than being invented.
    """
    payload = load_json(stdout)

    download_bps = coerce_int(payload.get("download"))
    upload_bps = coerce_int(payload.get("upload"))

    if download_bps is None and upload_bps is None:
        raise ParseError("The engine result contained no download or upload measurement.")

    host = coerce_str(dig(payload, "server", "host"))
    sponsor = coerce_str(dig(payload, "server", "sponsor"))
    city = coerce_str(dig(payload, "server", "name"))

    return EngineMeasurement(
        engine_name="speedtest-cli",
        engine_version=engine_version,
        download_bps=download_bps,
        upload_bps=upload_bps,
        idle_latency_ms=coerce_float(payload.get("ping")),
        # speedtest-cli measures no per-direction latency, jitter or loss.
        download_latency_ms=None,
        upload_latency_ms=None,
        jitter_ms=None,
        packet_loss_percent=None,
        server_id=coerce_str(dig(payload, "server", "id")),
        server_name=sponsor or city,
        server_host=host,
        server_city=city,
        server_region=None,
        server_country=coerce_str(dig(payload, "server", "country")),
        isp_name=coerce_str(dig(payload, "client", "isp")),
        external_ip=coerce_str(dig(payload, "client", "ip")),
        interface_name=None,
        raw_result_json=json.dumps(payload, sort_keys=True),
    )
